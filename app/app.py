...
def time_clock(time):
    ...
    if hour+":00" in day_tasks.keys():
        ...
        banner = [html.Div([
                    html.Div([aux.ret_img(day_tasks[hour+":00"].activity_type)]
                            ,style={'width':'40%'}),
                    html.Div([
                        html.H2(day_tasks[hour+":00"].activity_type),
                        html.Div("De " + hour+":00" + " a " + str(int(hour)+1) +":00 cal fer:"
                                ,style={'fontSize':20}), 
                        html.Br(), 
                        html.Div("- Activitat: " + day_tasks[hour+":00"].name
                                ,style={'fontSize':20}), 
                        html.Div("- Assignatura: " + day_tasks[hour+":00"].subject
                                ,style={'fontSize':20}),
                        html.Br()
...
def render_content(tab):
    if tab == 'avui':
        ...
        if str_date not in calendar.schedule.keys():
            return html.Div([
                        html.Div([
                            html.H1("No tens res a fer avui!")
        ...
        # for hour, task in day_tasks.items():
            lst.append(
                html.Div([
                    html.Button(hour + " | "+ task.name + " - "+ task.subject, id=str(task.date_time)
